...
import { LayoutDashboard, Building2, BookOpen, Tag, Users, Sun, Moon, Menu, X, LogOut } from "lucide-react";
...
const NAV = [
  ...
  { href: "/super-admin/tags", label: "Global Tags", Icon: Tag },
...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ ...vars, display: "flex", minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "'Sora','DM Sans',system-ui,sans-serif" }}>
      <aside className={`sa-sidebar${open ? " open" : ""}`} style={{ width: "220px", flexShrink: 0, background: "var(--surface)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 40, transition: "transform 0.22s ease" }}>
        <div style={{ padding: "1.25rem 1.25rem 1rem", borderBottom: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
            <div style={{ width: "2rem", height: "2rem", borderRadius: "0.5rem", background: "var(--accent-bg)", border: "1.5px solid var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.85rem", fontWeight: 900, color: "var(--accent)" }}>SA</div>
            <div>
              <div style={{ fontSize: "0.78rem", fontWeight: 800, color: "var(--text)" }}>Super Admin</div>
              <div style={{ fontSize: "0.6rem", color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Portal</div>
            </div>
          </div>
        </div>
     
        <nav style={{ flex: 1, padding: "0.75rem", display: "flex", flexDirection: "column", gap: "0.2rem" }}>
          {NAV.map(({ href, label, Icon, exact }: any) => {
      ...
      <div className="sa-main" style={{ flex: 1, marginLeft: "220px", display: "flex", flexDirection: "column" }}>
        <div className="sa-topbar" style={{ display: "none", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 1.25rem", background: "var(--surface)", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 30 }}>
          ...
          <span style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--accent)" }}>Super Admin</span>
