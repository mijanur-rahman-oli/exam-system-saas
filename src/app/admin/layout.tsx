...
import { LayoutDashboard, BookOpen, ClipboardList, GraduationCap, Tag, Users, Sun, Moon, Menu, X, LogOut, Book } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const tenantName = (session?.user as any)?.tenant?.name ?? "Admin";
  ...
  return (
    <div style={{ ...vars, display: "flex", minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "'Sora','DM Sans',system-ui,sans-serif" }}>
      <aside className={`adm-sidebar${open ? " open" : ""}`} style={{ width: "220px", flexShrink: 0, background: "var(--surface)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 40, transition: "transform 0.22s ease" }}>
        <div style={{ padding: "1.25rem 1.25rem 1rem", borderBottom: "1px solid var(--border)" }}>
          <div style={{ fontSize: "0.78rem", fontWeight: 800, color: "var(--text)" }}>{tenantName}</div>
          <div style={{ fontSize: "0.6rem", color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Admin Portal</div>
        </div>
      ...
      <div className="adm-main" style={{ flex: 1, marginLeft: "220px", display: "flex", flexDirection: "column" }}>
        <div className="adm-topbar" style={{ display: "none", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 1.25rem", background: "var(--surface)", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 30 }}>
          ...
          <span style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--accent)" }}>Admin</span>
