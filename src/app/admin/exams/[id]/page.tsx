...
import { ArrowLeft, Download, Edit, Trophy, ToggleLeft, ToggleRight, Trash2, BookOpen, Clock, Users, CheckCircle, XCircle, GraduationCap } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";

function stripLatex(t: string) {
      ...
      return `<div style="margin-bottom:20px;page-break-inside:avoid;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 24px;padding-left:22px;">
          ${opts.map(o=>`<div style="display:flex;gap:6px;align-items:flex-start;font-size:13px;line-height:1.5;">
            ...
            <div>${o.text?`<span>${stripLatex(o.text)}</span>`:""}${o.img?`<img src="${o.img}" style="max-height:80px;max-width:100%;object-fit:contain;display:block;margin-top:4px;"/>`:""}
            </div></div>`).join("")}
        </div>
      </div>`;
    }).join("");
             ...
             qs.map((eq:any,idx:number) => (
              <div key={eq.id} style={{ padding:"0.75rem 1.25rem",borderBottom:"1px solid var(--border)",display:"flex",gap:"0.75rem" }}>
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  <p style={{ fontSize:"0.8rem",color:"var(--text)",margin:"0 0 0.2rem",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{eq.question?.question}</p>
            ...
            <tbody>
              {qs.map((eq:any,idx:number)=>(
                <tr key={eq.id} style={{ borderBottom:"1px solid var(--border)" }}>
                  ...
                  <td style={{ padding:"0.75rem 1.1rem",color:"var(--text)",maxWidth:"320px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{eq.question?.question}</td>
