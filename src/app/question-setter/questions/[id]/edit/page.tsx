"use client";
/**
 * Edit Question page — works for both /admin/questions/[id]/edit
 * and /question-setter/questions/[id]/edit
 *
 * Pass the `backHref` prop or keep the default.
 * Images are shown from existing DB URLs and can be replaced or cleared.
 */
...
import { ImageUploadField } from "@/components/questions/ImageUploadFeild";
...
const textareaBase: React.CSSProperties = { ...inpBase, height: "auto", padding: "0.5rem 0.75rem", resize: "vertical" as any, lineHeight: "1.6" };
...
const schema = z.object({
  ...
  question:         z.string().min(5, "At least 5 characters"),
...
interface Props {
  /** Where "Back" and "Cancel" navigate to. Defaults to /question-setter/questions */
  backHref?: string;
}

export default function EditQuestionPage({ backHref = "/question-setter/questions" }: Props) {
  const { id }    = useParams();
  ...
  // ── Image state (new files) ───────────────────────────────────────────────
  const [questionImg,  setQuestionImg]  = useState<File | null>(null);
  const [solutionImg,  setSolutionImg]  = useState<File | null>(null);
  const [optionImgs,   setOptionImgs]   = useState<(File | null)[]>([null, null, null, null]);

  // ── Existing image URLs from DB (shown when no new file chosen) ───────────
  const [existingQuestionImg,  setExistingQuestionImg]  = useState<string | null>(null);
  const [existingSolutionImg,  setExistingSolutionImg]  = useState<string | null>(null);
  const [existingOptionImgs,   setExistingOptionImgs]   = useState<(string | null)[]>([null, null, null, null]);
  ...
  useEffect(() => {
    ...
    setExistingQuestionImg(question.questionImage ?? null);
    setExistingSolutionImg(question.solutionImage ?? null);
    setExistingOptionImgs([
      question.optionAImage ?? null,
      question.optionBImage ?? null,
      question.optionCImage ?? null,
      question.optionDImage ?? null,
    ]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Images: send new file OR "__keep__" to signal no change OR "__clear__" to remove
      const appendImg = (key: string, file: File | null, existing: string | null) => {
        if (file) { fd.append(key, file); }
        else if (existing) { fd.append(key, existing); } // keep existing
        else { fd.append(key, "__clear__"); }           // explicitly cleared
      };
      appendImg("questionImage", questionImg, existingQuestionImg);
      appendImg("solutionImage", solutionImg, existingSolutionImg);
      optionImgs.forEach((f, i) => appendImg(`optionImage_${i}`, f, existingOptionImgs[i]));

      const res = await fetch(`/api/questions/${id}`, { method: "PUT", body: fd });
      if (!res.ok) {
  ...
  if (qLoading) return <div style={{ padding: "3rem", textAlign: "center", color: "var(--text3)" }}>Loading question…</div>;
  if (!question) return <div style={{ padding: "3rem", textAlign: "center", color: "var(--red)" }}>Question not found.</div>;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <div>
            <h1 style={{ fontSize: "1.25rem", fontWeight: 800, margin: 0, color: "var(--text)" }}>Edit Question</h1>
            <p style={{ fontSize: "0.78rem", color: "var(--text3)", margin: 0 }}>ID #{id}</p>
          </div>
        </div>
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <Link href={backHref}>
            <button style={{ padding: "0.45rem 0.9rem", borderRadius: "0.5rem", fontSize: "0.8rem", background: "none", border: "1px solid var(--border)", color: "var(--text2)", cursor: "pointer" }}>
              Cancel
            </button>
          </Link>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <Card title="Question">
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              ...
              <textarea {...register("question")} rows={4} style={textareaBase}
              ...
              <div style={{ marginTop: "0.25rem" }}>
                <ImageUploadField
                  ...
                  value={questionImg}
                  existingUrl={existingQuestionImg}
                  onChange={setQuestionImg}
                  onClear={() => { setQuestionImg(null); setExistingQuestionImg(null); }}
                />
              </div>
            </div>
          </Card>
          ...
          <Card title="Answer Options">
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              {fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} style={{ border: "1px solid var(--border)", borderRadius: "0.625rem", overflow: "hidden" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.65rem", padding: "0.75rem" }}>
                      <Controller name={`options.${index}.isCorrect`} control={control} render={({ field: f }) => (
                        <button type="button"
                          ...
                          style={{ flexShrink: 0, width: "1.55rem", height: "1.55rem", borderRadius: "50%", border: `2px solid ${f.value ? "var(--green)" : "var(--border2)"}`, background: f.value ? "var(--green-bg)" : "transparent", color: f.value ? "var(--green)" : "var(--text3)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", fontWeight: 700, transition: "all 0.12s" }}>
                          {f.value ? <CheckCircle2 size={13} /> : letter}
                        </button>
                      )} />
                      <Controller name={`options.${index}.text`} control={control} render={({ field: f }) => (
                        <input value={f.value ?? ""} onChange={(e) => f.onChange(e.target.value)} placeholder={`Option ${letter}`}
                          style={{ ...inpBase, flex: 1 }} onFocus={(e) => (e.target.style.borderColor = "var(--accent)")} onBlur={(e) => (e.target.style.borderColor = "var(--border)")} />
                      )} />
                      {fields.length > 2 && (
                    ...
                    {/* Option image */}
                    <div style={{ padding: "0 0.75rem 0.75rem", borderTop: "1px dashed var(--border)" }}>
                      <ImageUploadField
                        label={`Option ${letter} image`}
                        value={optionImgs[index] ?? null}
                        existingUrl={existingOptionImgs[index]}
                        onChange={(f) => setOptionImgs((prev) => { const next = [...prev]; next[index] = f; return next; })}
                        onClear={() => {
                          setOptionImgs((prev) => { const next = [...prev]; next[index] = null; return next; });
                          setExistingOptionImgs((prev) => { const next = [...prev]; next[index] = null; return next; });
                        }}
                      />
                    </div>
                    {errors.options?.[index]?.text && (
                      <div style={{ padding: "0 0.75rem 0.5rem", fontSize: "0.7rem", color: "var(--red)" }}>{errors.options[index]?.text?.message}</div>
                    )}
                  </div>
                );
              })}
          ...
          <Card title="Explanation / Solution">
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                ...
                <textarea {...register("explanation")} rows={4} placeholder="Step-by-step solution…" style={textareaBase}
                  onFocus={(e) => (e.target.style.borderColor = "var(--accent)")} onBlur={(e) => (e.target.style.borderColor = "var(--border)")} />
              </div>
              <ImageUploadField
                label="Solution image"
                value={solutionImg}
                existingUrl={existingSolutionImg}
                onChange={setSolutionImg}
                onClear={() => { setSolutionImg(null); setExistingSolutionImg(null); }}
              />
            </div>
          </Card>
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                ...
                <Controller name="subjectId" control={control} render={({ field }) => (
                  <select value={field.value ?? ""} onChange={(e) => { field.onChange(e.target.value); setSelSubject(e.target.value); }} style={selBase}
                    onFocus={(e) => (e.target.style.borderColor = "var(--accent)")} onBlur={(e) => (e.target.style.borderColor = "var(--border)")}>
                    ...
                    {(["easy", "medium", "hard"] as const).map((d) => {
                      ...
                      return (
                        <button key={d} type="button" onClick={() => field.onChange(d)}
                          style={{ padding: "0.38rem", borderRadius: "0.4rem", fontSize: "0.7rem", fontWeight: active ? 700 : 500, textTransform: "capitalize", border: `1.5px solid ${active ? dc.border : "var(--border)"}`, background: active ? dc.bg : "var(--surface2)", color: active ? dc.text : "var(--text3)", cursor: "pointer", transition: "all 0.15s" }}>
                          {d}
                        </button>
                      );
                    })}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                  <Controller name="marks" control={control} render={({ field }) => (
                    <input type="number" min={1} max={100} value={field.value} onChange={(e) => field.onChange(Number(e.target.value))} style={inpBase}
                      onFocus={(e) => (e.target.style.borderColor = "var(--accent)")} onBlur={(e) => (e.target.style.borderColor = "var(--border)")} />
                  )} />
